    ...
    <div class="container mt-4">
        ...
        <form id="formRegistrasi" action="{{ route('register.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="namaUser" maxlength="15" required
                    value="{{ old('namaUser') }}">
            </div>
            ...
            <!-- Tambahkan di atas tombol -->
            <div class="mb-3">
                <label for="statusUser" class="form-label">Status Admin</label>
                <select name="statusUser" class="form-select" required>
                    <option value="Aktif" {{ old('statusUser') == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                    <option value="Nonaktif" {{ old('statusUser') == 'Nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                </select>
            </div>
